...
uint32_t nwsnleftNodes = 20;
uint32_t nwsnrightNodes = 20;
...
int main (int argc, char** argv)
{
  ...
  uint32_t payloadSize = 120;                       /* Transport layer payload size in bytes. */
  std::string tcpVariant = "TcpNewReno";             /* TCP variant type. */
  std::string link_speed = "2Mbps";
  std::string link_delay = "5ms";
  Time startTime = Seconds(0);
  Time stopTime = Seconds(100); /* Simulation time in seconds. */

  uint32_t noofflows = 16;
  uint32_t noofpackets_sec = 100;
  uint32_t speed = 10;
  ...
  cmd.AddValue("noofpackets_sec", "number of packets per second", noofpackets_sec);
...
//   wsnrightdevicesInterfaces.SetDefaultRouteInAllNodes (0);
  ...
  for(uint32_t i = 0; i < noofflows; i++){
    ...
    int receiver = (rand() % (nwsnrightNodes - 1));
  ...
  //   serverApp.Stop (stopTime);
    ...
    server.SetAttribute ("MaxBytes", UintegerValue (0));
    server.SetConstantRate (DataRate (noofpackets_sec*payloadSize*8));
