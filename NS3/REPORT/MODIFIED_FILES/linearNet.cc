...
main (int argc, char *argv[])
{
  ...
  uint32_t stopTime = 100; /* Simulation time in seconds. */


  double middlespeed = 1.5;
  double leftspeed = 10;
  double rightspeed = 10;
  double basertt = 26.012;
  ...
  cmd.AddValue ("middlespeed", "Enable pcap tracing", middlespeed);
  cmd.AddValue ("leftspeed", "Enable pcap tracing", leftspeed);
  cmd.AddValue ("rightspeed", "Enable pcap tracing", rightspeed);
  cmd.AddValue ("noofFlows", "no of flows", noofFlows);

  cmd.Parse (argc,argv);


  double temp = (1 / ((1 / middlespeed) + (1 / leftspeed) + (1 / rightspeed)));

  temp = temp * basertt;
  uint32_t ssthresh = ((uint32_t)(temp + 0.5)) * 1000;
...
//   anim.SetConstantPosition(wifiApNoderight.Get(0), 40.0, 80.0);
  ...
  // flowMonitor->SerializeToXmlFile("own_Linear_modifiedAlgo_topo_flow14000.xml", false, false);
  flowMonitor->SerializeToXmlFile("own_Linear_modifiedAlgo_topo_flow30000.xml", false, false);
  // flowMonitor->SerializeToXmlFile("own_Linear_withoutAlgo_topo_flow.xml", false, false);
  // flowMonitor->SerializeToXmlFile("own_Linear_modifiedalgo_topo_flow_withoutInitialthreshold.xml", false, false);
  Simulator::Destroy ();
