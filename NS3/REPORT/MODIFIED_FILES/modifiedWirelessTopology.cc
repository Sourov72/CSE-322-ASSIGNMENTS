...
main (int argc, char *argv[])
{
  bool verbose = true;
  bool tracing = true;
  uint32_t nWifileft = 20;
  uint32_t nWifiright = 20;
...
//   uint32_t packets_per_sec = 100;
  ...
  cmd.AddValue ("range", "maximum range of area", range);
...
//     }
  ...
  channelLeft.AddPropagationLoss ("ns3::RangePropagationLossModel", "MaxRange", DoubleValue (range * nWifileft));
...
//                              "Bounds", RectangleValue (Rectangle (-50, 50, -50, 50)));
  ...
  channelRight.AddPropagationLoss ("ns3::RangePropagationLossModel", "MaxRange", DoubleValue (range * nWifiright));
...
//   serverApp.Stop (stopTime);
  ...
  OnOffHelper server ("ns3::TcpSocketFactory", (InetSocketAddress (rightInterfaces.GetAddress (19), port)));
  ...
  ApplicationContainer serverApp = server.Install (wifileftNodes.Get (1));
  serverApp.Start(startTime);
  serverApp.Stop(stopTime);
  ...
  ApplicationContainer sinkApps = mysinkHelper.Install (wifirightNodes.Get (19));
...
for(uint32_t i = noofFlows /2; i < noofFlows; i++){
  ...
  OnOffHelper server ("ns3::TcpSocketFactory", (InetSocketAddress (leftInterfaces.GetAddress (19), port)));
  server.SetAttribute ("PacketSize", UintegerValue (payloadSize));
  server.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
  server.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
  server.SetAttribute ("DataRate", DataRateValue (DataRate (dataRate)));
  // server.SetConstantRate (DataRate (noofpacketspersec*payloadSize*8));
  ApplicationContainer serverApp = server.Install (wifirightNodes.Get (1));
  serverApp.Start (startTime);
  serverApp.Stop(stopTime);
  ...
  PacketSinkHelper mysinkHelper ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), port));
  ApplicationContainer sinkApps = mysinkHelper.Install (wifileftNodes.Get (19));
  Ptr<PacketSink> temp = StaticCast<PacketSink> (sinkApps.Get (0));
  sink.push_back(temp);
  lastTotalRx.push_back(0);
  sinkApps.Start (startTime);
